...
const Area = connect((state, props) => ({
  ...
  class Area extends Component {
    ...
    render() {
      ...
      const groupShipIds = groupBy(
        shipIds,
        id => (keyShips[id] || {}).superTypeIndex,
      )
      return (
        <div
          style={{ border: `solid 1px ${hexToRGBA(area.color, 0.5)}` }}
          className="area"
        >
          <div className="header">
            ...
            <div>
              <AddShipDropdown area={index} onSelect={this.handleAddShip} />
            </div>
          </div>
